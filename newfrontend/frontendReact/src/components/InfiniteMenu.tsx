...
const InfiniteMenu: FC<InfiniteMenuProps> = ({ items = [], scale = 1.0, onNavigate }) => {
  ...
  // Use refs to hold latest values without triggering re-creation
  ...
  // Throttle active item updates to reduce re-renders
  const lastActiveIndexRef = useRef<number>(-1);
  const activeItemRafRef = useRef<number | null>(null);
  ...
  const handleActiveItem = useCallback((index: number) => {
    ...
    pendingActiveIndexRef.current = itemIndex;

    if (activeItemRafRef.current === null) {
      activeItemRafRef.current = requestAnimationFrame(() => {
        const idx = pendingActiveIndexRef.current;
        if (idx !== null && idx !== lastActiveIndexRef.current) {
          lastActiveIndexRef.current = idx;
          setActiveItem(itemsRef.current[idx]);
        }
        activeItemRafRef.current = null;
      });
    }
  }, []);
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // Cleanup: stop the animation loop
      if (sketchRef.current) {
        sketchRef.current.destroy();
        sketchRef.current = null;
      }
      // Cancel any pending RAF for active item updates
      if (activeItemRafRef.current !== null) {
        cancelAnimationFrame(activeItemRafRef.current);
        activeItemRafRef.current = null;
      }
    };
  }, [handleActiveItem]);

  // Update WebGL texture when items change
  useEffect(() => {
